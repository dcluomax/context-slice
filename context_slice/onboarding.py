...
def instruction_text(root: Path) -> bytes:
    ...
    return (
        ...
        "For the first applicable local lookup in a session, use "
        "`prepare QUERY --root ROOT --scope TOPIC --session SESSION_ID --max-bytes 8192`. "
        "This combines bounded retrieval with an explicit session-use receipt. "
        "The receipt verifies the runtime and records use; it does not infer that "
        "instructions were read. `session-status --session SESSION_ID` is read-only. "
        "Only pass `--ack-instructions SHA256` after actually reading those instructions. "
        "Later lookups may use `brief QUERY --root ROOT --scope TOPIC --max-bytes 8192`. "
        ...
        "Batch independent reads and use deterministic scripts for repeated preparation; "
        "do not add a model round trip merely to reformat local data. Keep long tasks "
        "scoped and preserve durable handoffs if automatic compaction fails; never "
        "disable compaction or globally downgrade models to hide a latency problem. "
